...
| **Inbound webhook** | `POST /hooks-pro/:slug` — HMAC + timestamp window + nonce de-dup, rate-limited |
| **Manual** | A **Run** button on the flow list, or a content-item toolbar action |
| **Connector event** | Polling a feed or JSON endpoint on an interval (see below) |
| **Agent** | The Studio Agent invokes a Flow as a tool |

#### Connector-event modes

A connector-event trigger runs in one of two modes:

- **`poll`** — the Hub fetches a URL on an interval and starts one run per new item, tracked by a cursor so nothing fires twice. The first poll only establishes a baseline, so activating a trigger on an existing feed does not replay its history. This covers RSS/Atom feeds, JSON endpoints, and status APIs.
- **`engine`** — a native push trigger compiled into the flow engine (a Slack message, a Stripe event). Stored and validated, but **not yet active**.

<Callout kind="info">
  An `engine`-mode trigger reports `active: false` with a reason in the API and the triggers dialog, so a configured automation never looks armed while it is still a no-op. Use `poll` mode for anything that must fire today.
</Callout>

<Callout kind="tip">
  Schedule and poll triggers claim each firing atomically, so a two-node deployment fires them exactly once rather than twice.
</Callout>
...
| Variable | Purpose |
|---|---|
| `INTEGRATIONS_KEK` | Key-encryption key that wraps each org's vault data key (AES-256-GCM). **Required in production** — the vault refuses to start without it rather than using a derivable dev key. Generate with `openssl rand -base64 32` |
...
<Steps>
  ...
  <Step title="Set the vault key" icon="key">
    Set `INTEGRATIONS_KEK` to a base64 32-byte key (`openssl rand -base64 32`). Every credential is encrypted at rest with a per-org data key wrapped by this KEK. In production the plugin will not start without it.
  </Step>
