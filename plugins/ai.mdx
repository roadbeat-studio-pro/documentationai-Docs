...
When `pro-ai` is active, editors get an **AI Assistant** sidebar and per-field **✨ actions** directly in the content editor — quick actions, chat, one-click translation, and alt-text suggestions, each shown as an accept/reject diff so nothing is ever applied silently.

<Card title="Using AI in the editor" icon="sparkles" href="https://docs.studio.roadbeat.dev/guides/ai-assistant" horizontal={true} cta="Open the editor guide">
...
All endpoints live under `/api/v1/pro/ai` and are documented in the [Studio API Reference](https://docs.studio.roadbeat.dev/api-reference/overview).
