...
description: >-
  One governed gateway for local + cloud AI — chat, translation, writing
  assistant, embeddings, vision, and schema-grounded generation, with routing,
  fallback, and per-call metering.
---
## Overview
...
| `vision`                          | Image → alt text, caption, OCR, tags, or description — alt-text suggestions are generated in the **organization's default content locale** and carry a locale tag | Alt-text on upload          |
...
<Steps>
  <Step title="Open AI settings" icon="settings" title-type="p">
    Go to **AI** in the Pro section of the sidebar and switch to the **Providers** tab.
  </Step>

  <Step title="Choose a provider type" icon="plug" title-type="p">
  ...
  <Step title="Set residency, scope, and priority" icon="shield" title-type="p">
  ...
  <Step title="Test the connection" icon="activity" title-type="p">
...
<Card title="Using AI in the editor" href="https://studio-docs.roadbeat.net/guides/ai-assistant" icon="sparkles" cta="Open the editor guide" horizontal="true">
...
<Callout kind="info" collapsed="false">
  Non-EU providers are gated at **routing** time: without `ai_cloud_providers_enabled`, EU/local providers still route normally, and a call is only refused when its _only_ eligible providers are non-EU. See [Licensing](/licensing) for tiers and offline validation.
</Callout>
